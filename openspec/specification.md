...
- **数据可视化**: Recharts 2.12 (响应式 SVG 图表)
- **AI 集成**: Google GenAI SDK (Gemini 3 Flash) + 离线降级逻辑
- **PWA 支持**: Vite PWA Plugin (离线缓存, 安装到主屏幕)
- **国际化**: 模块化 i18n 方案 (按语言拆分文件，支持 10 种语言)
- **状态管理**: React Hooks (useState, useMemo, useRef)
- **URL 同步**: 所有筛选状态通过 URL 参数持久化
...
- **交互**: 
  ...
  - **多场景评分**: 支持 balanced (平衡)、developer (开发者)、creative (创意)、daily (日常) 四种评分权重模式。
  - **PC 参照系**: 支持一键混入 PC 顶级硬件数据作为性能锚点。
  - **冻结列**: 移动端横向滚动时，机型名称列固定在左侧，保持上下文。
  - 点击行弹出详情模态框。
- **对比选择**: 提供勾选框，允许用户选择最多 2 款机型进行对比。
- **URL 同步**: 所有筛选、排序、对比状态通过 URL 参数持久化，支持分享。
...
- **模式切换**:
  ...
  - **Points/$**: 每美元性能得分排行。
- **参照系**: 图表中也会显示开启的 PC 参照硬件，使用斜线填充图案区分。
...
- **特性**: 
  - 支持 Markdown 渲染。
  - 多语言回复。
  - **离线降级**: 网络不可用时，通过本地关键词匹配提供基础建议。
- **优化**: 设置 `thinkingBudget: 0` 关闭思维链以提高响应速度。
...
- **依赖版本锁定**: 使用 package.json 锁定版本。
- **代码规范**: 每个文件头部包含版本号注释 (vX.Y.Z)。
- **本地化**: 所有界面字符串都在 `lib/locales/` 中定义，支持 10 种语言。
